 ...
 * Uso: node build-packages.js disciplinas.csv [CDN_BASE]
 * Requer: npm install archiver
 */
...
var MANIFEST_TEMPLATE = fs.readFileSync(
  path.join(ROOT_DIR, "imsmanifest.xml"),
  "utf8"
);
...
function buildPackage(row) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (!CDN_BASE) {
      archive.file(path.join(ROOT_DIR, "style.css"), { name: "style.css" });
      archive.file(path.join(ROOT_DIR, "scorm.js"), { name: "scorm.js" });
      archive.directory(path.join(ROOT_DIR, "css"), "css");
      archive.directory(path.join(ROOT_DIR, "js"), "js");
    }
